...
def _open_image(file_obj):
    ...
    # Handle GIF (animated): use first frame as static avatar
...
def _apply_crop(img, crop: Optional[Dict[str, float]]):
    # crop expects dict with x, y, size, scale applied on original image space
    if not crop:
    ...
    # Apply scale by resizing beforehand, then crop a square of `size`
    w, h = img.size
...
def _to_rgb(img):
    # Convert to RGB – if has alpha, paste on white
    from PIL import Image  # ensure Image symbol available in this scope
    if img.mode in ("RGB",):
        return img
...
def process_profile_image(file_obj, crop: Optional[Dict[str, float]] = None, size: int = 512) -> bytes:
    """
    Validates and processes uploaded image: crop -> resize -> optimize -> JPEG bytes
    """
    _ensure_pillow()
    ...
    if file_size > 5 * 1024 * 1024:
        raise ValueError("이미지 크기는 최대 5MB까지 가능합니다.")
    file_obj.seek(0)
...
def upload_to_s3_and_get_url(user_id: int, image_bytes: bytes, ext: str = "jpg") -> str:
    """Uploads to S3 under profiles/{user_id}.{ext} and returns public URL.
    Requires AWS credentials and bucket configuration in settings.
    """
    import base64
    ...
    if not (bucket and region and access_key and secret_key):
        raise RuntimeError("S3 설정이 누락되었습니다. 환경변수와 settings를 확인하세요.")

    try:
        import boto3
    except Exception as e:  # pragma: no cover
        raise RuntimeError("boto3 패키지가 필요합니다. 'pip install boto3'로 설치하세요.") from e
